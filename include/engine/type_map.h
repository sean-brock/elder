...
public:
  ...
  template <class Key> inline iterator find() {
    return m_map.find(getTypeId<Key>());
  }

  template <class Key> inline bool contains() {
    return m_map.contains(getTypeId<Key>());
  }

  // Same as above, const version
  template <class Key> inline const_iterator find() const {
    return m_map.find(getTypeId<Key>());
  }

  // Associates a value with the type "Key"
  template <class Key> inline void put(ValueType &&value) {
...
template <class ValueType> std::atomic_int TypeMap<ValueType>::LastTypeId(0);
